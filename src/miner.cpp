...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("AstronMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake()) {
            ...
            pwalletMain->zdevTracker->ListMints(true, true, true); //update the state
        }
        return error("AstronMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("AstronMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("astron-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running AstronMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),

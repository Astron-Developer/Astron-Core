...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Astron address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no Astron: URI
...
{
    // Convert astron:// to astron:
    //
    //    Cannot handle this later, because astron:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open astron.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Astron.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Astron.lnk
...
{
    return GetAutostartDir() / "astron.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a astron.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Astron\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the astron app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add astron app to startup item list

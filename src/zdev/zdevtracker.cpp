...
CoinWitnessData* CzDEVTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzDEVTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
{
    bool iszDEVWalletInitialized = (NULL != zDEVWallet);
    CMintMeta meta;
    ...
    if (! iszDEVWalletInitialized)
        zDEVWallet = new CzDEVWallet(strWalletFile);
...
std::set<CMintMeta> CzDEVTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    ...
    if (fUpdateStatus) {
        ...
        CzDEVWallet* zDEVWallet = new CzDEVWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zDEVWallet);
        }
        delete zDEVWallet;

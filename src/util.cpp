...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "astron" is a composite category enabling all Astron-related debug output
            if (ptrCategory->count(string("astron"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Astron
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Astron
// Mac: ~/Library/Application Support/Astron
// Unix: ~/.astron
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Astron";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "astron.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override astron.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "astrond.pid"));

...
#if defined(NDEBUG)
#error "Astron cannot be compiled without assertions."
#endif
...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_Public_Spend_Enabled()) {
        ...
        if (v1Serial) {
            return error("%s : zDEV v1 serial spend not spendable, serial %s, tx %s\n", __func__,
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            // Check that zDEV mints (if included) are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
     nSubsidy = 36313353 * COIN;
    }
	else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight > 0) {
     nSubsidy = 50 * COIN;
    }
	else if (nHeight <= 90002 && nHeight > Params().LAST_POW_BLOCK()) {
     nSubsidy = 40 * COIN;
    }
	else if (nHeight <= 135002 && nHeight > 90002) {
     nSubsidy = 30 * COIN;
    }
	else if (nHeight <= 660602 && nHeight > 135002) {
     nSubsidy = 28 * COIN;
    }
	else if (nHeight <= 1186202 && nHeight > 660602) {
     nSubsidy = 24 * COIN;
    }
	else if (nHeight <= 1711802 && nHeight > 1186202) {
     nSubsidy = 20 * COIN;
    }
	else if (nHeight <= 2763002 && nHeight > 1711802) {
     nSubsidy = 19 * COIN;
    }
	else if (nHeight <= 3814202 && nHeight > 2763002) {
     nSubsidy = 17 * COIN;
    }
	else if (nHeight <= 5391002 && nHeight > 3814202) {
     nSubsidy = 15 * COIN;
    }
	else if (nHeight <= 7493402 && nHeight > 5391002) {
     nSubsidy = 13 * COIN;
    }
	else if (nHeight <= 9595802 && nHeight > 7493402) {
     nSubsidy = 10 * COIN;
    }
	else if (nHeight <= 11698202 && nHeight > 9595802) {
     nSubsidy = 9 * COIN;
    }
	else if (nHeight <= 13800602 && nHeight > 11698202) {
     nSubsidy = 8 * COIN;
    }
	else if (nHeight <= 18005402 && nHeight > 13800602) {
     nSubsidy = 7 * COIN;
    }
	else if (nHeight <= 22210202 && nHeight > 18005402) {
     nSubsidy = 6 * COIN;
    }
	else if (nHeight <= 26415002 && nHeight > 22210202) {
     nSubsidy = 5 * COIN;
    }
	else if (nHeight > 26415002) {
     nSubsidy = 0 * COIN;
    }
	else {
     nSubsidy = 0 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZDEVStake)
{
	...
	else if (nHeight <= 660602 && nHeight > Params().LAST_POW_BLOCK()) {
		ret = blockValue * 0.95;
	}
	else if (nHeight <= 2763002 && nHeight > 660602) {
		ret = blockValue * 0.85;
	}
	else if (nHeight <= 5391002 && nHeight > 2763002) {
		ret = blockValue * 0.80;
	}
	else if (nHeight <= 9595802 && nHeight > 5391002) {
		ret = blockValue * 0.75;
	}
	else if (nHeight <= 13800602 && nHeight > 9595802) {
		ret = blockValue * 0.73;
	}
	else if (nHeight <= 18005402 && nHeight > 13800602) {
		ret = blockValue * 0.70;
	}
	else if (nHeight <= 26415002 && nHeight > 18005402) {
		ret = blockValue * 0.65;
	}
	else if (nHeight > 26415002) {
		ret = blockValue * 0.60;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Astron
...
void RecalculateZDEVMinted()
{
    ...
    uiInterface.ShowProgress(_("Recalculating minted ZDEV..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZDEV..."), percent);
        }
...
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZDEV..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZDEV..."), percent);
        }
...
bool UpdateZDEVSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!UpdateZDEVSupply(block, pindex, fJustCheck))
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        if (isBlockFromFork) {
            ...
            if(hasZDEVInputs){
                for (const CTxIn& zPivInput : zDEVInputs) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Astron: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zdev", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zdev", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
